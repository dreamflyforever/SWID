...
#define MIN_SNR 1.35f
#define TOTAL_MIN_SNR 1
#define RANGE_MIN_SNR 1.35f
#define LONG_RANGE_MIN_SNR 1.4f
#define MIN_WEAK_SNR 1.0f
...
struct EventInfo *ei_init2(struct EventInfo *_this, tidx timeIdx, struct FreqAmplitude *p1, struct FreqAmplitude *p2)
{
	...
	_this->uPoints.idxPoints.p1 = *p1;
	_this->uPoints.idxPoints.p2 = *p2;
	return _this;
}
...
{
	memset(_this->uidexs.idxes, -1, sizeof(_this->uidexs.idxes));
	_this->flags = 0;
}
...
bool fi_checkFreq2(struct FrequencyInfo *_this, struct EventInfo *_event, float _minMatchFreqYDistance)
{
	...
	else
	{
		...
		struct FreqAmplitude *freqs = _event->uPoints.points;
		...
		if (_freq != 0)
		{
			...
			if (times_size >= 2 * maxMainMatchCount
				...
				&& (times_last->frequency->amplitude * 3 < _freq->amplitude || times_last2->frequency->amplitude * 3 < _freq->amplitude))
			{
				_this->endReason = 5;
				return false;
			}
			...
			if (times_size >= 2 && times_last2->frequency->amplitude > times_last->frequency->amplitude && ((times_last->frequency->amplitude > _freq->amplitude) || times_last2->frequency->amplitude > _freq->amplitude))
			{
				downing = true;
			}
...
struct FrequencyInfo *fi_removeSmallMatch(struct FrequencyInfo *_this, int _maxMatchCount)
{
	...
	if (fi_timesLast(_this)->event->timeIdx - fi_timesFirst(_this)->event->timeIdx + 1 > _maxMatchCount)
	{
		...
		times_size = fi_timesCount(_this);
		leftIdx = (_this->maxOrderAmplitudeIdx >= 0?_this->maxOrderAmplitudeIdx:_this->maxAmplitudeIdx);
		rightIdx = leftIdx;
...
void fi_addTime(struct FrequencyInfo *_this, const struct FrequencyMatch *_fm)
{
	...
	if (_this->maxAmplitudeIdx < 0 ||  _fm->frequency->amplitude > matchTimes[_this->maxAmplitudeIdx].frequency->amplitude)
	{
		assert(_this->maxAmplitudeIdx < times_size);
		_this->maxAmplitudeIdx = times_size - 1;
	}
	if (_fm->frequency->order == 1 && (_this->maxOrderAmplitudeIdx < 0  || _fm->frequency->amplitude > matchTimes[_this->maxOrderAmplitudeIdx].frequency->amplitude))
	{
		assert(_this->maxOrderAmplitudeIdx < times_size);
		_this->maxOrderAmplitudeIdx = times_size - 1;
	}
...
{
	return (*((struct FrequencyInfo **)p_data_left))->frequencyY - (*((struct FrequencyInfo **)p_data_right))->frequencyY;
}
...
{
	fari_initNoiseLevel(_this, _initTop2Avg);
	_this->inited = false;
...
void fari_onFFTAmpsRange(struct FFTAmpsRangeTop2AvgIniter *_this, struct FFTAmpsRange *_amps, struct Idx2EventInfo *_idx2EventInfo, struct FreqAmpsEventStat *_eventStat)
{
	if (!_this->inited && _eventStat->avgAmplitude > 0.0005)
	{
		...
		if (_this->rangeCurrAvgCount >= RANGE_TOP2AVGINITER_AVG_SIZE)
		{
			...
			_this->rangAvgSNR = (_this->rangAvgSNR * (_this->rangeCurrAvgCount-1)+amplitudeTop2Avg)/_this->rangeCurrAvgCount;
		}
		else
		{
			_this->rangAvgSNR = (_this->rangAvgSNR * _this->rangeCurrAvgCount + amplitudeTop2Avg)/(_this->rangeCurrAvgCount+1);
		...
		if (_this->rangAvgSNR < _this->minPeakAmplitudeTop2Avg)
		{
			...
			{
				fari_initNoiseLevel(_this, _this->sumPeakAmplitudeTop2Avg/_this->sumPeakAmplitudeTop2AvgCount);

#if !defined(SILENCE) && !defined(PRINT_INIT_NOISE)
				myprintf("signalAnalyser init:%f\n", (_this->sumPeakAmplitudeTop2Avg/_this->sumPeakAmplitudeTop2AvgCount));
#endif
			}
		}
		else
...
void fari_initNoiseLevel(struct FFTAmpsRangeTop2AvgIniter *_this, float normalTop2Avg)
{
	if(normalTop2Avg == 0)
	{
		_this->inited = false;
	...
	{
		_this->normalAmplitudeTop2Avg = normalTop2Avg;
		_this->minPeakAmplitudeTop2Avg = _this->normalAmplitudeTop2Avg * MIN_SNR;
		_this->minRangePeakAmplitudeTop2Avg = _this->normalAmplitudeTop2Avg * RANGE_MIN_SNR;
		_this->minLongRangePeakAmplitudeTop2Avg = _this->normalAmplitudeTop2Avg * LONG_RANGE_MIN_SNR;
		_this->minWeakPeakAmplitudeTop2Avg = _this->normalAmplitudeTop2Avg * MIN_WEAK_SNR;
		_this->inited = true;
	}
}
...
void fis_onFFTEvent(struct FrequencyInfoSearcher *_this, struct EventInfo *_ei, float _maxFreqYDistance)
{
...

	{
		...
		faValidAmplitudes = _ei->uPoints.points;
...
int bs_removeWeakestBlock(struct BlockSearcher *_this, int _startBlockIdx, int _blockCount, struct FrequencyInfo*_removedFI)
{
...
void bs_onFFTEvent(struct BlockSearcher *_this, struct EventInfo *_ei)
{
	...
	else
	{
		...
		assert(_this->unsedBlockIdx < BLOCKSEARCH_BLOCK_CACHE_COUNT && (_this->unsedBlockIdx < 0 || _this->unsedBlockIdx == _this->okBlockIdx));
	}

	if (_ei != NULL)
	{
		if ((_ei->uPoints.idxPoints.p1.frequencyY != 0 && abs(_ei->uPoints.idxPoints.p1.frequencyY - _this->blockFreqY) <= 1 && _ei->uPoints.idxPoints.p1.peakType != 3)
			|| (_ei->uPoints.idxPoints.p2.frequencyY != 0 && abs(_ei->uPoints.idxPoints.p2.frequencyY - _this->blockFreqY) <= 1 && _ei->uPoints.idxPoints.p2.peakType != 3)
#ifndef MIN_MEMORY
			|| (_ei->uPoints.idxPoints.p3.frequencyY != 0 && abs(_ei->uPoints.idxPoints.p3.frequencyY - _this->blockFreqY) <= 1 && _ei->uPoints.idxPoints.p3.peakType != 3)
#endif
			)
		{
			hasBlockSignal = true;
	...
	if((_ei == NULL && fi_realTimesCount(&_this->blockFIs[0]) > 0) 
		|| ((hasBlockSignal || fi_realTimesCount(&_this->blockFIs[0]) > 0) && !fi_checkFreq2(&_this->blockFIs[0], _ei, 1)))

	{
			...
			)
		{
			...
			if (_this->blockCount > 0 
				...
				&& fi_timesFirst(&_this->blockFIs[0])->event->timeIdx - fi_timesFirst(&_this->blockFIs[1])->event->timeIdx < ((1+RS_CORRECT_SIZE) * preciseMatchCount)
				)
			{
			...
			else
			{
				...
				{
					bool s2_1OK = (fi_timesFirst(&_this->blockFIs[1])->event->timeIdx - fi_timesFirst(&_this->blockFIs[2])->event->timeIdx) >= ((RS_CORRECT_BLOCK_SIZE+RS_CORRECT_SIZE) * preciseMatchCount);
				...
				{
					tidx s2_1_Len = (fi_timesFirst(&_this->blockFIs[1])->event->timeIdx - fi_timesFirst(&_this->blockFIs[2])->event->timeIdx);
			...
			if (removedFIIdx != 0)
			{
				...
				_this->blockFIs[(removedFIIdx>0?1:0)].blockSignalCount = pd_preBlockPeakCount(&_this->peakDetector);
			}
			if (removedFIIdx == 1)
			{
	...
	if (_ei == NULL 
		&& _this->blockCount >= 2
		&& ((fi_timesFirst(&_this->blockFIs[_this->okBlockIdx])->event->timeIdx - fi_timesFirst(&_this->blockFIs[_this->okBlockIdx+1])->event->timeIdx) < ((RS_CORRECT_BLOCK_SIZE+RS_CORRECT_SIZE-1) * preciseMatchCount)))
	{

		bool lastBlockInvalid = _this->blockFIs[_this->okBlockIdx].blockSignalCount < 2 
			|| (_this->blockFIs[_this->okBlockIdx].blockSignalCount == 2 && sumFIAmplitude(&_this->blockFIs[_this->okBlockIdx]) < sumFIAmplitude(&_this->blockFIs[_this->okBlockIdx+1]) * 0.2);
		if (lastBlockInvalid)
		{
...
{
	if (_ei != NULL && _ei->uPoints.idxPoints.p1.frequencyY != 0)
	{
		int idx = getIdxFromFrequencyFromCache(_this->sa, _ei->uPoints.idxPoints.p1.frequencyY);
		if (idx >= 0 && idx < DEFAULT_CODE_FREQUENCY_SIZE)
		{
			if ((_ei->timeIdx - _this->lastPosTimes[idx]) > (RangeMatchCount * 3/2))
			{
				...
				_this->peakStartTimes[pos].frequencyY = _ei->uPoints.idxPoints.p1.frequencyY;
				_this->nextPeakPos ++;
...
void pd_onBlock(struct PeakDetector *_this, struct FrequencyInfo *_blockFI)
{
	...
	for (i = _this->nextPeakPos-1; i >= minIdx; i --)
	{
		...
		if (_this->peakStartTimes[pos].timeIdx <= blockEndPos && abs(_this->peakStartTimes[pos].frequencyY - _this->blockFreqY) <= 1)
		{
			break;
		}
	}
...
void fats_onFFTAmpsRange(struct FreqAmpsEventStat *_this, struct FFTAmpsRange *_amps, float _minPeak2Avg, float _minStatPeak2Avg, float _maxFreqYDistance)
{
	...
	mymemset(_this->event.uPoints.points, 0, MAX_EVENT_AMP * sizeof(struct FreqAmplitude *));
	...
	if (_this->freqY2FreqIdxSize != (maxFreqIdx - minFreqIdx + 1))
	{
		...
		_this->freqY2FreqIdx = (cchar *)mymalloc(_this->freqY2FreqIdxSize);
	...
	hasValidAmplitude = maxFreqAmplitude->amplitude / ((_this->longTermAvgAmplitude > avgAmplitude)?avgAmplitude:_this->longTermAvgAmplitude) > _minPeak2Avg*TOTAL_MIN_SNR;
	...
	{
		_this->longTermAvgAmplitude = (_this->longTermAvgAmplitude * _this->longTermAvgAmpCount + avgAmplitude)/(_this->longTermAvgAmpCount+1);
		_this->longTermAvgAmpCount ++;
	}
	else
	{
		_this->longTermAvgAmplitude = (_this->longTermAvgAmplitude * (_this->longTermAvgAmpCount-1) + avgAmplitude)/_this->longTermAvgAmpCount;
	}
	...
	{
		_this->event.uPoints.idxPoints.p1 = *maxFreqAmplitude;
		...
		if (max2FreqAmplitude != NULL && max2FreqAmplitude->amplitude >= minValidPeak)
		{
			...
			_this->event.uPoints.idxPoints.p2 = *max2FreqAmplitude;
			if (max2_max > 0.5)
			{
				_this->event.uPoints.idxPoints.p1.peakType = 2;
			}
#ifndef MIN_MEMORY
			if (max3FreqAmplitude != NULL && max3FreqAmplitude->amplitude >= minValidPeak)
			{
				...
				{
					_this->event.uPoints.idxPoints.p3 = *max3FreqAmplitude;
				}
			}
			else if(max3FreqAmplitude != NULL && max3FreqAmplitude->amplitude >= minStatPeak)
			{
				_this->event.uPoints.idxPoints.p3 = *max3FreqAmplitude;
				_this->event.uPoints.idxPoints.p3.peakType = 3;
			}
#endif
		}
		...
		{
			_this->event.uPoints.idxPoints.p2 = *max2FreqAmplitude;
			_this->event.uPoints.idxPoints.p2.peakType = 3;
		}
	}
}
...
struct SignalAnalyser *sa_init(struct SignalAnalyser *_this, int _sampleRate, int _channel, int _bits, int _fftSize, int _overlap)
{
	...
	vector_init2(&_this->similarSignals, sizeof(struct TimeRangeSignal), NULL, MAX_CHARS);
	_this->blockCount = 0;
...
int getIdxFromFrequency(struct SignalAnalyser *_this, int _pitch)
{
	...
	int pos = mybinarySearch_(&_pitch, _this->CODE_Y + 1, DEFAULT_CODE_FREQUENCY_SIZE - 1 - 1, sizeof(int), intComparator);
...
{
	int i, minIdx, maxIdx, minFrequency, maxFrequency;
	float freqPerIdx = ((float)(_this->sampleRate))/_this->fftSize;
	...
	minFrequency = _this->minFrequency - 3 * freqPerIdx;
	minFrequency = ((int)(minFrequency / freqPerIdx)) * freqPerIdx;
	_this->maxFrequency = minFrequency + (_this->maxFrequency - _this->minFrequency + 2 * 3 * freqPerIdx);
	_this->minFrequency = minFrequency;
	...
	if (_this->freqY2IdxCacheCount != (maxIdx - minIdx + 1))
	{
		if (_this->freqY2IdxCaches != NULL)myfree(_this->freqY2IdxCaches);
		_this->freqY2IdxCacheCount = maxIdx - minIdx + 1;
		assert(_this->freqY2IdxCacheCount < 150);
		_this->freqY2IdxCaches = (cchar *)mymalloc(_this->freqY2IdxCacheCount);
	}
	_this->freqY2IdxCacheMinIdx = minIdx;
	assert(_this->freqY2IdxCaches != NULL);
...
bool sa_isInited(struct SignalAnalyser *_this, float *_noiseLevel)
{
	if(_noiseLevel != NULL && _this->top2AvgIniter.inited)
	{
		*_noiseLevel = _this->top2AvgIniter.normalAmplitudeTop2Avg;
	}
	return _this->top2AvgIniter.inited;
}

void sa_reInit(struct SignalAnalyser *_this, float _noiseLevel)
{
	fari_initNoiseLevel(&_this->top2AvgIniter, _noiseLevel);
}
...
int getFrequencyInfoEventDeviation(int _freqY, tidx _eventStart, tidx _eventEnd, float _matchCount, int _signalEventIdxOff, bool _printLog)
{
	...
	tidx fiRangeStart = getRangeStart(rangeIdx, _matchCount, _signalEventIdxOff), fiRangeEnd = getRangeEnd(rangeIdx, _matchCount, _signalEventIdxOff);
	if(_printLog)myprintf("range %d (%d, %d) cal diviation (%d, %d)\n", rangeIdx, fiRangeStart, fiRangeEnd, _eventStart, _eventEnd);

	if ((fiRangeStart-_eventStart >= 0 && fiRangeEnd - _eventEnd <= 0) || (fiRangeStart-_eventStart <= 0 && fiRangeEnd - _eventEnd >= 0))
	{
		absMinus = 0;
	}
	else
...
struct FreqAmplitude *matchEventAmplitude(struct EventInfo *_event, int _freqY, float _minYDistance)
{
	if (_event != NULL)
	{
		if (_event->uPoints.idxPoints.p1.frequencyY != 0)
		{
			if(abs(_event->uPoints.idxPoints.p1.frequencyY - _freqY) <= _minYDistance)
			{
				return &_event->uPoints.idxPoints.p1;
			}
			else if (_event->uPoints.idxPoints.p2.frequencyY != 0)
			{
				if(abs(_event->uPoints.idxPoints.p2.frequencyY - _freqY) <= _minYDistance)
				{
					return &_event->uPoints.idxPoints.p2;
				}
#ifndef MIN_MEMORY
				else if (_event->uPoints.idxPoints.p3.frequencyY != 0)
				{
					if(abs(_event->uPoints.idxPoints.p3.frequencyY - _freqY) <= _minYDistance)
					{
						return &_event->uPoints.idxPoints.p3;
					}
				}
#endif
			}
...
int sa_analyEventsIdxOff(struct SignalAnalyser *_this, tidx _startEvent, tidx _endEvent, tidx _maxEndEvent, float _matchCount)
{
	...
	for (i = _startEvent; i <= _maxEndEvent+1; i ++)
	{
		...
		if (preEvent == NULL || preEvent->uPoints.idxPoints.p1.frequencyY == 0 || currEvent == NULL || currEvent->uPoints.idxPoints.p1.frequencyY == 0)
		{
			isEqual = false;
		}
		else
		{
			isEqual = abs(currEvent->uPoints.idxPoints.p1.frequencyY - preEvent->uPoints.idxPoints.p1.frequencyY) <= _this->maxFreqYDistance;
		}
		if (isEqual)
		{
			if(sameCount == 0)
			...
			{
				if (currEvent->uPoints.idxPoints.p1.amplitude > siganlMaxEvent->uPoints.idxPoints.p1.amplitude)
				{
					siganlMaxEvent = currEvent;
				}
			}
		...
		else
		{
			if (sameCount+1 >= MIN_SIGNAL_BIG_MATCH)
			{
				...
				matchSignalEvents(_this, siganlMaxEvent->uPoints.idxPoints.p1.frequencyY, siganlMaxEvent->timeIdx, &signalStartEvent, &signalEndEvent);
				for (j = 0; j < searchMinRangeCount; j ++)
				{
					eventDeviation = getFrequencyInfoEventDeviation(siganlMaxEvent->uPoints.idxPoints.p1.frequencyY, signalStartEvent->timeIdx, signalEndEvent->timeIdx, _matchCount, j, false);
			...
			else if (spanSameCount > 0)
			{
				...
				if (prepreEvent == NULL || prepreEvent->uPoints.idxPoints.p1.frequencyY == 0 || currEvent == NULL || currEvent->uPoints.idxPoints.p1.frequencyY == 0)
				{
					curr2prepreEqual = false;
				}
				else
				{
					curr2prepreEqual = abs(currEvent->uPoints.idxPoints.p1.frequencyY - prepreEvent->uPoints.idxPoints.p1.frequencyY) <= _this->maxFreqYDistance;
				}
				if (curr2prepreEqual)
				{
					assert(abs(siganlMaxEvent->uPoints.idxPoints.p1.frequencyY-currEvent->uPoints.idxPoints.p1.frequencyY) <= _this->maxFreqYDistance);
					assert(abs(signalStartEvent->uPoints.idxPoints.p1.frequencyY-currEvent->uPoints.idxPoints.p1.frequencyY) <= _this->maxFreqYDistance);
					if (currEvent->uPoints.idxPoints.p1.amplitude > siganlMaxEvent->uPoints.idxPoints.p1.amplitude)
					{
						siganlMaxEvent = currEvent;
					}
	...
	if(minDeviationIdx < 0)minDeviationIdx = searchImin(sumSignalDeviations, sizeof(sumSignalDeviations)/sizeof(int));
	return minDeviationIdx;
}

void addEventAmpToFreqRanges(struct SignalAnalyser *_this, struct FIMatch *_freqRanges, struct EventInfo *_e, struct FreqAmplitude *_amp, int _idx)
{
...
void getTimeRangeFrequencyPeak(struct SignalAnalyser *_this, tidx _timeStart, tidx _timeEnd, struct FIMatch *_freqRanges)
{
	tidx i, idx;
	struct EventInfo *e;
	...
	for (i = _timeStart; i <= _timeEnd; i ++)
	{
		...
		if(e->uPoints.idxPoints.p1.frequencyY != 0)addEventAmpToFreqRanges(_this, _freqRanges, e, &e->uPoints.idxPoints.p1, i - _timeStart);
		if(e->uPoints.idxPoints.p2.frequencyY != 0)addEventAmpToFreqRanges(_this, _freqRanges, e, &e->uPoints.idxPoints.p2, i - _timeStart);
#ifndef MIN_MEMORY 
		if(e->uPoints.idxPoints.p3.frequencyY != 0)addEventAmpToFreqRanges(_this, _freqRanges, e, &e->uPoints.idxPoints.p3, i - _timeStart);
#endif
	}
}
...
float getAroundAmplitudeDis(struct SignalAnalyser *_this, int _freqY, tidx _startEventIdx, int _step)
{
	...
	{		
		...
		if (e != NULL)
		{
			if(e->uPoints.idxPoints.p1.frequencyY != 0)
			{
				if(abs(e->uPoints.idxPoints.p1.frequencyY - _freqY) <= _this->maxFreqYDistance)
				{
					aroundSumAmplitude += e->uPoints.idxPoints.p1.amplitude;
					hasAmp = true;
				}
				else if(e->uPoints.idxPoints.p2.frequencyY != 0)
				{
					if(abs(e->uPoints.idxPoints.p2.frequencyY - _freqY) <= _this->maxFreqYDistance)
					{
						aroundSumAmplitude += e->uPoints.idxPoints.p2.amplitude;
						hasAmp = true;
					}
#ifndef MIN_MEMORY
					else if(e->uPoints.idxPoints.p3.frequencyY != 0)
					{
						if(abs(e->uPoints.idxPoints.p3.frequencyY - _freqY) <= _this->maxFreqYDistance)
						{
							aroundSumAmplitude += e->uPoints.idxPoints.p3.amplitude;
							hasAmp = true;
						}
					}
#endif
...
void topNTimeRangeMatch(struct TimeRangeMatch *_matches, int _matchCount, int _rangeIdx, int _peak, struct FIMatch *_match)
{
	int i;
...
int sa_analyValidSignals3(struct SignalAnalyser *_this, int _eventStart, int _eventEnd, bool _isPreciseEnd, bool _isFullBlock)
{
...

	{
		...
		int blockBestIdxOff = sa_analyEventsIdxOff(_this, _eventStart,
			(((_eventStart + 5 * preciseMatchCount)<_eventEnd)?(_eventStart + 5 * preciseMatchCount):_eventEnd),
			(((_eventStart + 10 * preciseMatchCount)<_eventEnd)?(_eventStart + 10 * preciseMatchCount):_eventEnd),
			oneSignalLen);
		...
		for (i = 0; i < blockSignalCount; i ++)
		{
			...
			for (j = 0; j < DEFAULT_CODE_FREQUENCY_SIZE; j ++)
			{
				...
				jPeak = _this->freqRanges[j].bigMatchCount * 10 + _this->freqRanges[j].headMatchCount * 2 + (_this->freqRanges[j].orderMatchCount - _this->freqRanges[j].tinyMatchCount) * 6 + _this->freqRanges[j].tinyMatchCount * 3;
			...
			for (j = 0; j < MAX_FI_MATCH_COUNT; j ++)
			{
				...
				if (curr2pre > 2)
				{
					if (targetPeakType == 0)
					{
						...
						if (curMatchs[j].sumQuality/curMatchs[j].sumAmplitude >= (rangeIdxEnd-rangeIdxStart+1) * 0.75)
						{
							freqPeakType = 2;
						}
					}
				...
				else if (curr2pre < 0.5)
				{
					...
					if (curMatchs[j].rangeIdx == DEFAULT_START_TOKEN
						...
						&& ((j == 0)
						|| curMatchs[j].bigMatchCount + curMatchs[j].smallMatchCount >= (rangeIdxEnd-rangeIdxStart+1)/2
						)
				...
				else
				{
					...
					if (curMatchs[j].rangeIdx == prepreTargetIdx && curMatchs[j].bigMatchCount >= 3 && curr2pre > 1.5 && preMatchs[k].sumQuality/preMatchs[k].sumAmplitude >= (rangeIdxEnd-rangeIdxStart))
					{
						freqPeakType = 1;
					}
				...
				if (freqPeakType == 1 || freqPeakType == 2)
				{
					...
					else if (curMatchs[j].rangeIdx == prepreTargetIdx && curMatchs[j].rangeIdx != DEFAULT_OVERLAP_TOKEN)
					{
				...
				if (i >= 2 && i < blockSignalCount-1 && curMatchs[j].rangeIdx == DEFAULT_START_TOKEN)
				{
					freqPeakType = 3;
				}
				...
				if (targetIdx == DEFAULT_START_TOKEN && preTargetIdx != DEFAULT_START_TOKEN && curMatchs[j].sumAmplitude/curMatchs[matchIdx].sumAmplitude < 0.2)
				{
					freqPeakType = 3;
				}

				if (freqPeakType == 2  && curMatchs[j].sumQuality/curMatchs[j].sumAmplitude > (rangeIdxEnd-rangeIdxStart+1) * 0.82)
				{
					freqPeakType = 3;
				}
				...
				if (freqPeakType == 1 || freqPeakType == 2)
				{
					...
					if (targetIdx < 0 
						...
						&& (curMatchs[j].bigMatchCount+curMatchs[j].smallMatchCount>(rangeIdxEnd-rangeIdxStart+1)*0.8)
						&& (curMatchs[j].preMatchIdx < 0 || preMatchs[curMatchs[j].preMatchIdx].sumQuality/preMatchs[curMatchs[j].preMatchIdx].sumAmplitude>(rangeIdxEnd-rangeIdxStart+1)*0.75))))
						&& similarCount < MAX_RANGE_IDX_COUNT)
					{

						if (targetIdx >= 0
							&& curMatchs[j].bigMatchCount+curMatchs[j].smallMatchCount>(rangeIdxEnd-rangeIdxStart+1)*0.8
							&& curMatchs[matchIdx].sumQuality/curMatchs[matchIdx].sumAmplitude>(rangeIdxEnd-rangeIdxStart+1)*0.75
							&& preMatchs[curMatchs[j].preMatchIdx].sumQuality/preMatchs[curMatchs[j].preMatchIdx].sumAmplitude>(rangeIdxEnd-rangeIdxStart+1)*0.75)
						{
							targetPeakType = 2;
							matchIdx = j;
							targetIdx = curMatchs[j].rangeIdx;
							currSignalIdx.uidexs.idxes[0] = curMatchs[j].rangeIdx;										
							signalIdxInPreMatches.uidexs.idxes[0] = j;
							similarCount = 1;

						}
						else if((targetIdx < 0 || targetPeakType == 2) && similarCount < MAX_RANGE_IDX_COUNT)
						{
							currSignalIdx.uidexs.idxes[similarCount] = curMatchs[j].rangeIdx;
							signalIdxInPreMatches.uidexs.idxes[similarCount] = j;
							similarCount++;
			...
			{
				schar tempSignalIdx = currSignalIdx.uidexs.idxes[0];
				currSignalIdx.uidexs.idxes[0] = currSignalIdx.uidexs.idxes[1];
				currSignalIdx.uidexs.idxes[1] = tempSignalIdx;
				tempSignalIdx = signalIdxInPreMatches.uidexs.idxes[0];
				signalIdxInPreMatches.uidexs.idxes[0] = signalIdxInPreMatches.uidexs.idxes[1];
				signalIdxInPreMatches.uidexs.idxes[1] = tempSignalIdx;
				targetIdx = currSignalIdx.uidexs.idxes[0];
				matchIdx = signalIdxInPreMatches.uidexs.idxes[0];
			}
				...
				|| (i == blockSignalCount-1 && targetIdx == DEFAULT_START_TOKEN))
			{
				...
				currSignalIdx.uidexs.idxes[similarCount] = curMatchs[guessIdx].rangeIdx;
				if(currSignalIdx.uidexs.idxes[similarCount] < 0)currSignalIdx.uidexs.idxes[similarCount] = 1;
				signalIdxInPreMatches.uidexs.idxes[similarCount] = guessIdx;
				similarCount ++;
				...
				&& ((targetPeakType == 1) || (targetPeakType == 2 && similarCount == 1)))
			{
				...
				{					
					...
					{
						if (preRangeSignal->uidexs.idxes[prex] < 0)break;
						if (preRangeSignal->uidexs.idxes[prex] == targetIdx)
						{
							assert(preSignalIdxInPreMatches.uidexs.idxes[prex] >= 0);
							if (preMatchs[preSignalIdxInPreMatches.uidexs.idxes[prex]].sumQuality/preMatchs[preSignalIdxInPreMatches.uidexs.idxes[prex]].sumAmplitude > (rangeIdxEnd-rangeIdxStart+1) * 0.75)
							{

								schar oldIdx1 = preRangeSignal->uidexs.idxes[prex];
								preRangeSignal->uidexs.idxes[prex] = -1;
								if (MAX_RANGE_IDX_COUNT-prex-1>0)
								{
									memmove(preRangeSignal->uidexs.idxes+prex, preRangeSignal->uidexs.idxes+prex+1, MAX_RANGE_IDX_COUNT-prex-1);
									preRangeSignal->uidexs.idxes[MAX_RANGE_IDX_COUNT-1] = -1;
								}											

								if (preRangeSignal->uidexs.idxes[0] == DEFAULT_START_TOKEN && preRangeSignal->uidexs.idxes[1] == -1)
								{
									trs_setMaybeError(preRangeSignal, true);
									preRangeSignal->uidexs.idxes[1] = oldIdx1;
								}
							}
							else
							{
								...
								if (prex < preSimilarCount-1)
								{
									...
									schar tempChar = preRangeSignal->uidexs.idxes[prex];
									preRangeSignal->uidexs.idxes[prex] = preRangeSignal->uidexs.idxes[preSimilarCount-1];
									preRangeSignal->uidexs.idxes[preSimilarCount-1] = tempChar;
								}
							}
				...
				if (preTaretPeakType == 2 && preRangeSignal->uidexs.idxIdxs.idx1 == targetIdx)
				{
					trs_setMaybeError(preRangeSignal, true);
				}
			}
			...
			if(targetIdx >= 0)
			{
				...
				matchSignalEvents(_this, _this->CODE_Y[targetIdx], rangeIdxStart + _this->freqRanges[targetIdx].topAmplitudeIdx, 
			...
			if(targetIdx < 0)
			{
				...
				currSignalIdx.uidexs.idxIdxs.idx1 = targetIdx;
				currSignalIdx.uidexs.idxIdxs.idx2 = curMatchs[((curMatchs[matchIdx+1].rangeIdx==preTargetIdx)?(matchIdx+2):(matchIdx+1))].rangeIdx;
				...
				if (i >= 2 && i < blockSignalCount-1 && currSignalIdx.uidexs.idxIdxs.idx2 == DEFAULT_START_TOKEN)
				{
					currSignalIdx.uidexs.idxIdxs.idx2 = -1;
				}
				...
				if (targetIdx < 0 || (targetIdx == 0 && i == blockSignalCount-1))
				{
					...
					targetIdx = -1;
					currSignalIdx.uidexs.idxIdxs.idx1 = ((i == blockSignalCount-1)?1:0);
				}
				similarCount = ((currSignalIdx.uidexs.idxIdxs.idx2 >= 0)?2:1);
			}

			if ((currSignalIdx.uidexs.idxIdxs.idx1 == DEFAULT_OVERLAP_TOKEN || currSignalIdx.uidexs.idxIdxs.idx2 == DEFAULT_OVERLAP_TOKEN))
			{
				if (vector_size(&_this->similarSignals) >= 1)
				{
					struct TimeRangeSignal *preRangeSignal = (struct TimeRangeSignal *)vector_last(&_this->similarSignals);

					if ((preRangeSignal->uidexs.idxIdxs.idx1 == DEFAULT_OVERLAP2_TOKEN || preRangeSignal->uidexs.idxIdxs.idx2 == DEFAULT_OVERLAP2_TOKEN))
					{
						if (vector_size(&_this->similarSignals) >= 2)
						{
							struct TimeRangeSignal *prePreRangeSignal = (struct TimeRangeSignal *)vector_at(&_this->similarSignals, vector_size(&_this->similarSignals) - 2);
							if (trs_maybeError(prePreRangeSignal))
							{
								trs_setMaybeError(&currSignalIdx, true);
							}
						}
					...
					{
						if (trs_maybeError(preRangeSignal))
						{
							trs_setMaybeError(&currSignalIdx, true);
						}
					}
			...
			if ((currSignalIdx.uidexs.idxIdxs.idx1 == DEFAULT_OVERLAP2_TOKEN || currSignalIdx.uidexs.idxIdxs.idx2 == DEFAULT_OVERLAP2_TOKEN))
			{
				if (vector_size(&_this->similarSignals) >= 2)
				{
					struct TimeRangeSignal *prePreRangeSignal = (struct TimeRangeSignal *)vector_at(&_this->similarSignals, vector_size(&_this->similarSignals) - 2);
		...
		_this->blocks[_this->blockCount].signalCount = vector_size(&_this->similarSignals) - _this->blocks[_this->blockCount].startIdx;
...
void sa_analyFFTSignal(struct SignalAnalyser *_this)
{
	...
	if ((int)vector_size(&_this->similarSignals) >= _this->minRegcoSignalCount && _this->blockCount > 0)
	{
		if(recogStatus == Recog_Success)
		{
			...
			{
				float soundTime = (_this->overlap+((float)(lastSignalIdx)) * (_this->fftSize - _this->overlap))/_this->sampleRate;
				bool thisTimeOK = false;
#ifdef SEARCH_SIMILAR_SIGNAL
				thisTimeOK = _this->listener->onStopRecognition2(_this->listener, soundTime, recogStatus, vector_nativep(&_this->similarSignals), vector_size(&_this->similarSignals)
					, _this->blocks, _this->blockCount);
#else
	...
	if (_this->listener != NULL && recogStatus != Recog_Success)
	{
		if (_this->recoging)
		{
			float soundTime = (_this->overlap+((float)(iei_idx(&_this->idx2Times)-1)) * (_this->fftSize - _this->overlap))/_this->sampleRate;
...
void sa_analyFFT(struct SignalAnalyser *_this, float amplitudes[])
{
	...
	if (!finished)
	{
		...
		fats_onFFTAmpsRange(&_this->eventStat, &_this->ampsRange, _this->top2AvgIniter.minPeakAmplitudeTop2Avg, _this->top2AvgIniter.minWeakPeakAmplitudeTop2Avg, _this->maxFreqYDistance);		

		iei_add(&_this->idx2Times, &_this->eventStat.event);
		buffedEvent = iei_curr(&_this->idx2Times);
		currEventIdx = iei_idx(&_this->idx2Times);
		if(!_this->top2AvgIniter.inited)fari_onFFTAmpsRange(&_this->top2AvgIniter, &_this->ampsRange, &_this->idx2Times, &_this->eventStat);

		if(_this->eventStat.hasValidAmplitude
			&& ((currEventIdx - _this->preValidFreqAmplitudeTime >= MAX_SIGNAL_SHORT_VALID_SIZE) || _this->firstValidFreqAmplitudeTime < 0))
		...
		if (_this->rangeCurrAvgCount >= RANGE_AVG_SIZE)
		{
			...
			_this->rangAvgSNR = (_this->rangAvgSNR * (_this->rangeCurrAvgCount-1) + _this->eventStat.amplitudeTop2Avg)/_this->rangeCurrAvgCount;
		}
		else
		{
			_this->rangAvgSNR = (_this->rangAvgSNR * _this->rangeCurrAvgCount + _this->eventStat.amplitudeTop2Avg)/(_this->rangeCurrAvgCount+1);
			_this->rangeCurrAvgCount ++; 
		}
		...
		freqTruncate =
			((_this->recoging && _this->firstRangeValidFreqAmplitudeTime < (currEventIdx - (MAX_EVENT_COUNT - 10)))
				|| vector_size(&_this->similarSignals) > MAX_CHARS
				|| ((_this->firstValidFreqAmplitudeTime<0||currEventIdx-_this->firstValidFreqAmplitudeTime<=MAX_SHORT_SIGNAL_SIZE)?false:
				(
	...
	while (bs_hasBlock(&_this->blockSearcher, &blockHead, &blockTail, &isEndBlock))
	{
		...
		if (_this->recoging)
		{
			...
			sa_analyValidSignals3(_this, fi_timesFirst(blockHead)->event->timeIdx, fi_timesFirst(blockTail)->event->timeIdx, true, !isEndBlock);
		...
		&& (currEventIdx - _this->firstValidFreqAmplitudeTime) > MIN_START_SIGNAL_SIZE && !freqTruncate)
	{
		...
		{
			float soundTime = (_this->overlap+((float)(_this->firstRangeValidFreqAmplitudeTime)) * (_this->fftSize - _this->overlap))/_this->sampleRate;
