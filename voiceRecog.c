...
{
	mrl_finalize((struct MyRecognitionListener *)_this);
}

struct GeneralRecognitionListener *grl_init(struct GeneralRecognitionListener *_this, void *_listener, 
	vr_pRecognizerStartListener _startListener, vr_pRecognizerEndListener _endListener, vr_pRecognizerMatchListener _matchListener)
{
...
{
	struct VoiceRecognizer *recognizer = (struct VoiceRecognizer *)_recognizer;
	vrr_setFreqs(recognizer, 2, _freqs, _freqCount);
}
...
VOICERECOGNIZEDLL_API vr_bool vr_isVoiceRecognizerInited(void *_recognizer, int _freqIdx, float *_noiseLevel)
{
	...
	struct VoiceRecognizer *recognizer = (struct VoiceRecognizer *)_recognizer;
	return (vrr_isInited(recognizer, _freqIdx, _noiseLevel)?vr_true:vr_false);
#else
	return vr_true;
#endif
}
...
#ifdef CONTROL_RECOGNIZER_INIT
	struct VoiceRecognizer *recognizer = (struct VoiceRecognizer *)_recognizer;
	vrr_reInit(recognizer, _freqIdx, _noiseLevel);
#endif
}
	...
	vr_pRecognizerEndListener _endListener, vr_pRecognizerMatchListener _matchListener)
{
	...
	if (listener == NULL)
	{
		vrr_setListener(recognizer, _idx, listener = (struct RecognitionListener *)_new(struct GeneralRecognitionListener, grl_init, _listener, _startListener, _endListener, _matchListener));
	}
	else
	{
...
void vr_setRecognizerFreq3Listener(void *_recognizer, void *_listener, vr_pRecognizerStartListener _startListener, 
	vr_pRecognizerEndListener _endListener, vr_pRecognizerMatchListener _matchListener)
{
	vr_setRecognizerListenerImple(_recognizer, 2, _listener, _startListener, _endListener, _matchListener);
}
...
{
	struct VoiceRecognizer *recognizer = (struct VoiceRecognizer *)_recognizer;
	struct BufferDataWriter *writer = vrr_getBufferWriter(recognizer, false);
	if(writer != NULL)bdw_flush(writer);
	vrr_flush(recognizer);
...
VOICERECOGNIZEDLL_API int vr_writeData(void *_recognizer, cchar *_data, int _dataLen)
{
	struct VoiceRecognizer *recognizer = (struct VoiceRecognizer *)_recognizer;
	struct BufferDataWriter *writer = vrr_getBufferWriter(recognizer, true);
	return bdw_write(writer, _data, _dataLen);
}

void vr_nextWriteBuff(void *_recognizer, char **_buf, int *_bufSize)
{
	struct VoiceRecognizer *recognizer = (struct VoiceRecognizer *)_recognizer;
	vrr_nextWriteBuff(recognizer, _buf, _bufSize);
}

short *sbuf = NULL;
#define FBUF_SIZE (1024 * 10)
VOICERECOGNIZEDLL_API int vr_writefData(void *_recognizer, cchar *_data, int _dataLen)
{
	struct VoiceRecognizer *recognizer = (struct VoiceRecognizer *)_recognizer;
	struct BufferDataWriter *writer = vrr_getBufferWriter(recognizer, true);
	float *fdata = (float *)_data;
	int fdataLen = _dataLen/sizeof(float);
	int fbufCount = ((fdataLen%FBUF_SIZE>0)?(fdataLen/FBUF_SIZE+1):(fdataLen/FBUF_SIZE));
	int i, ibuf, ibufSize;

	if (sbuf == NULL)sbuf = (short *)mymalloc(FBUF_SIZE * sizeof(short));
	for (ibuf = 0; ibuf < fbufCount; ibuf ++)
	{
		ibufSize = ((ibuf == (fbufCount-1) && ((fdataLen%FBUF_SIZE)>0))?(fdataLen%FBUF_SIZE):(FBUF_SIZE));
		for (i = 0; i < ibufSize; i ++)
		{
			sbuf[i] =  fdata[ibuf*FBUF_SIZE+i] * 32767; 
		}
		bdw_write(writer, (char *)sbuf, ibufSize * sizeof(short));
	}
...
int hex2Digit(cchar *_hexs, int _hexsLen, int *_costHexsLen, cchar *_result, int _resultLen, int _resultBufSize)
{
	int i;
...
inline int hex2Chars(cchar *_hexs, int _hexsCount, int *_costHexsCount, cchar *_result, int _maxBitCount, int _resultBufSize)
{
...
int hex2Lower(cchar *_hexs, int _hexsLen, int *_costHexsLen, cchar *_result, int _resultLen, int _resultBufSize)
{
	cchar *tempChars = (cchar *)mymalloc(sizeof(cchar) * (_hexsLen/2+1));
	int resultIdx;
	int costBits = 0;
	asize1(_result, _resultBufSize);

	hex2Chars(_hexs, _hexsLen, _costHexsLen, tempChars, ((_resultLen>0)?_resultLen * 5:_resultLen), sizeof(cchar) * (_hexsLen/2+1));
...
int hex2Upper(cchar *_hexs, int _hexsLen, int *_costHexsLen, cchar *_result, int _resultLen, int _resultBufSize)
{
	cchar *tempChars = (cchar *)mymalloc(sizeof(cchar) * (_hexsLen/2+1));
	int resultIdx;
	int costBits = 0;
...
int hex2Char64(cchar *_hexs, int _hexsLen, int *_costHexsLen, cchar *_result, int _resultLen, int _resultBufSize)
{
	cchar *tempChars = (cchar *)mymalloc(sizeof(cchar) * (_hexsLen/2+1));
	int resultIdx;
	int costBits = 0;
	...
	hex2Chars(_hexs, _hexsLen, _costHexsLen, tempChars, ((_resultLen>0)?_resultLen * 6:_resultLen), sizeof(cchar) * (_hexsLen/2+1));
	for (resultIdx = 0; (_resultLen < 0 || resultIdx < _resultLen); resultIdx ++)
	{
...
int hex2Char256(cchar *_hexs, int _hexsLen, int *_costHexsLen, cchar *_result, int _resultLen, int _resultBufSize)
{
	return hex2Chars(_hexs, _hexsLen, _costHexsLen, _result, ((_resultLen>0)?_resultLen * 8:_resultLen), _resultBufSize);
}

inline int hex2Type(cchar *_hexs, int _hexsLen, int *_hexsCostLen, cchar *_type, int *_aPartLen)
{
	int r = 1;
...
int vr_decodeData(cchar *_hexs, int _hexsLen, int *_hexsCostLen, cchar *_result, int _resultLen, int _resultBufSize)
{	
	...
	*_hexsCostLen = 0;
	...
	if (type == 1)
	{
		...
		dataLen += hex2Lower(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, aPartLen, _resultBufSize);
	...
	else if (type == 2)
	{
		...
		dataLen += hex2Char64(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, aPartLen, _resultBufSize);
		*_hexsCostLen += costHexsAdd;
		dataLen += hex2Digit(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result + dataLen, 
			((_resultLen<0)?_resultLen:(_resultLen-dataLen)), _resultBufSize);
		*_hexsCostLen += costHexsAdd;
	...
	else if (type == 4)
	{
		...
		dataLen += hex2Digit(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, aPartLen, _resultBufSize);
		*_hexsCostLen += costHexsAdd;
	...
	else if (type == 0)
	{
		...
		dataLen += hex2Digit(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, _resultLen, _resultBufSize);
		*_hexsCostLen += costHexsAdd;
	}
	else if (type == 3)
	{

		dataLen += hex2Char256(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, _resultLen, _resultBufSize);
		*_hexsCostLen += costHexsAdd;
	}
	else if (type == 5)
	{

		dataLen += hex2Lower(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, _resultLen, _resultBufSize);
		*_hexsCostLen += costHexsAdd;
	}
	else if (type == 6)
	{

		dataLen += hex2Char64(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, _resultLen, _resultBufSize);
		*_hexsCostLen += costHexsAdd;
	}
	else if (type == 7)
	{

		dataLen += hex2Upper(_hexs + *_hexsCostLen, _hexsLen - *_hexsCostLen, &costHexsAdd, _result, _resultLen, _resultBufSize);
		*_hexsCostLen += costHexsAdd;
	}
	return dataLen;
}

vr_bool vr_decodeString(int _recogStatus, cchar *_data, int _dataLen, cchar *_result, int _resultLen)
{
...
VOICERECOGNIZEDLL_API vr_bool vr_decodeBytes(int _recogStatus, cchar *_data, int _dataLen, cchar *_result, int _resultLen, int *_returnLen)
{
	int costHexsLen = 1, costHexsAdd = 0;
	int i;
	asize1(_result, _resultLen);
	assert(vr_decodeInfoType(_data, _dataLen) == IT_BYTES);
	for (i = 0; i < (_dataLen-costHexsLen)/2 && i < _resultLen; i ++)
	{
		_result[i] = (hexChar2Int(_data[costHexsLen+i*2]) << 4) | hexChar2Int(_data[costHexsLen+i*2+1]);
	}
	*_returnLen = i;
	return vr_true;
}

enum InfoType vr_decodeInfoType(cchar *_data, int _dataLen)
{
	asize1(_data, _dataLen);
	return (enum InfoType)(hexChar2Int(adata(_data, 0)) >> 1);
}

vr_bool vr_decodeWiFi(int _result, cchar *_data, int _dataLen, struct WiFiInfo *_wifi)
{
	...
	int dataLen = vr_decodeData(_data+costHexsLen, _dataLen-costHexsLen, &costHexsAdd, _wifi->mac, macLen, sizeof(_wifi->mac));
	assert(dataLen == macLen);
	_wifi->macLen = macLen;
	costHexsLen += costHexsAdd;
	dataLen = vr_decodeData(_data+costHexsLen, _dataLen-costHexsLen, &costHexsAdd, _wifi->pwd, -1, sizeof(_wifi->pwd));
	adata(_wifi->pwd, dataLen) = 0;
	_wifi->pwdLen = dataLen;
	return vr_true;
}

vr_bool vr_decodeSSIDWiFi(int _result, cchar *_data, int _dataLen, struct SSIDWiFiInfo *_wifi)
{
	...
	dataLen = vr_decodeData(_data+costHexsLen, _dataLen-costHexsLen, &costHexsAdd, _wifi->ssid, ssidLen, sizeof(_wifi->ssid));
...
vr_bool vr_decodeSSIDWiFi2(int _result, cchar *_data, int _dataLen, struct SSIDWiFiInfo2 *_wifi)
{

	asize3(_data, _dataLen, _wifi->ssid, sizeof(_wifi->ssid), _wifi->pwd, sizeof(_wifi->pwd))
	int ssidLen = (((hexChar2Int(adata(_data, 0)) & 0x01) << 4) | hexChar2Int(adata(_data, 1))) + 1;
	int costHexsLen = 3, costHexsAdd = 0;
	int dataLen = 0;
	assert(vr_decodeInfoType(_data, _dataLen) == IT_SSID_WIFI);
	_wifi->encType = hexChar2Int(adata(_data, 2));
	dataLen = vr_decodeData(_data+costHexsLen, _dataLen-costHexsLen, &costHexsAdd, _wifi->ssid, ssidLen, sizeof(_wifi->ssid));
	assert(dataLen == ssidLen);
	adata(_wifi->ssid, dataLen) = 0;
	_wifi->ssidLen = ssidLen;
...
int vr_bs_composeBlock(struct BlocksComposer *_this, cchar *_blockData, int _blockDataLen)
{
