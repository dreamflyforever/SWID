...
#define MAX_BLOCK_COUNT 15
#define maxMainMatchCount 6

#define MAX_EVENT_COUNT ((MAX_BLOCK_COUNT*(VOICE_BLOCK_SIZE+1+1)+2)*maxMainMatchCount)
#define MAX_CHARS (MAX_BLOCK_COUNT*(VOICE_BLOCK_SIZE+1+1)+2)
...
struct TimeRangeSignal
{
	union
	{
		schar idxes[MAX_RANGE_IDX_COUNT];
		struct
		{
			schar idx1;
			schar idx2;
...
{
	short frequencyY;
	cchar preciseFreqCount;	
	cchar shortFreqCount;
	cchar prePeakType;
	cchar preOrderType;	
	cchar realBigMatchCount;
	cchar bigMatchCount;
	cchar orderMatchCount;
	struct FrequencyMatch realTimes[maxFIEventCount];
	cchar realEventCount, timesStart, timesEnd;
	schar maxAmplitudeIdx, maxOrderAmplitudeIdx;
	cchar preDowning;

	cchar endReason;
	cchar blockSignalCount;
	tidx startTimeIdx;
	int idx;
};
...
{	
	...
	float sumQuality;
	cchar bigMatchCount;
	cchar smallMatchCount;
	cchar tinyMatchCount;
	short rangeIdx;
	short preMatchIdx;
};
...
void sa_setMinRegcoSignalCount(struct SignalAnalyser *_this, int minRegcoSignalCount);
