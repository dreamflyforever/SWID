...
void * pa_new(struct PoolAllocator *_this)
{
	...
	cchar *currBlock = (cchar *)((vector_size(&_this->blocks) > currBlockIdx)?(*(void **)vector_at(&_this->blocks, currBlockIdx)):NULL);
	if (currBlock == NULL)
	{
		currBlock = (cchar *)mymalloc(_this->sizeofT * PoolAllocatorObjCountPerBlock);
