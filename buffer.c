...
inline cchar *bd_getData(struct BufferData * _this)
{
	if (_this->mData == NULL && _this->mMaxBufferSize > 0)
	{
		_this->mData = (cchar *)mymalloc(sizeof(cchar) *_this->mMaxBufferSize);
	}
	return _this->mData;
}
...
const cchar * Buffer_TAG = "struct Buffer";
...
struct BufferData *b_tryGetFull(struct Buffer *_this)
{
	struct BufferData *data = NULL;
	...
	mtx_lock(&_this->m);
#endif
	if (cq_try_dequeue(&_this->mConsumeQueue, &data))
	{
		cnd_broadcast(&_this->cond);
	}
#ifndef COND_NO_MUTEX
